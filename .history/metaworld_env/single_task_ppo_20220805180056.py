...
def train_policy2(agent, storage, optimizer, batch_size, T_HORIZON, GAMMA, LAMBDA, EPS_CLIP, ENT_COEFF):
    obs, actions, rewards, next_obs, old_log_pobs, values = storage.get_batch()
    td_target = rewards + GAMMA * agent.get_value(next_obs)
    delta = td_target - values
    with torch.no_grad():
...
for e in range(10000):
    ...
    while step < max_length and not done:
        env.render()
        with torch.no_grad():
            action, log_prob  = agent.sample_action(obs)  # Sample an action
