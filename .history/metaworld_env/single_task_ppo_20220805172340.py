...
def train_policy(agent, storage, batch_size, T_HORIZON, GAMMA, LAMBDA, EPS_CLIP, ENT_COEFF):
    ...
    for i in range(K_EPOCHS):
        for s_, a_, r_, ns_, old_log_p_, v_, adv_, ret_ in agent.choose_mini_batch(advantages, returns):
            _, cur_log_p, entorpy, _ = agent.get_action_prob_entropy_value(s_)
            ratio = torch.exp()
