...
class PPO(nn.Module):
    ...
    def pi(self, obs, softmax_dim=1):
        hidden = self.forward(obs)
        mu = 2 * torch.tanh(self.actor_mean(hidden))
        #sigma = F.softplus(self.actor_logstd(hidden)) +1e-3
        return mu, self.actor_logstd

    def sample_action(self, obs):
        prob, value = self.pi(obs, softmax_dim=1)
        prob = prob.squeeze(0) # Remove batch dim
        m = Categorical(prob)
        action_index = m.sample().item()
        return prob, action_index

    def sample_action_prob(self, )
