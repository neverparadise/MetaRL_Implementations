...
def train_policy(agent, storage, batch_size, T_HORIZON, GAMMA, LAMBDA):
    obs, actions, rewards, next_obs, log_pobs, values = storage.get_batch()
    ...
    with torch.no_grad():
        ...
        for i in range(K_EPOCHS):
            for s_, a_, r_, ns_, log_p_, v_, adv_, ret_ in agent.choose_mini_batch(advantages, returns):
