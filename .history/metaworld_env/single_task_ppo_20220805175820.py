...
class PPO(nn.Module):
    ...
    def sample_action(self, obs):
        mu, std = self.pi(obs, softmax_dim=1)
        prob = Normal(mu, std)
        prob = prob.squeeze(0) # Remove batch dim
        action_index = prob.sample().item()
...
def train_policy(agent, storage, optimizer, batch_size, T_HORIZON, GAMMA, LAMBDA, EPS_CLIP, ENT_COEFF):
    ...
    for i in range(K_EPOCHS):
        for s_, a_, r_, ns_, old_log_p_, v_, adv_, ret_ in storage.choose_mini_batch(batch_size, advantages, returns):
            _, cur_log_p, entropy, cur_v = agent.get_action_prob_entropy_value(s_)
...
for e in range(10000):
    ...
    while step < max_length and not done:
        env.render()
        with torch.no_grad():
            action_index, prob  = agent.sample_action(obs)  # Sample an action

        next_obs, reward, done, info = env.step(action_index.cpu().numpy())  # Step the environoment with the sampled random action
        step += 1
        next_obs = obs
